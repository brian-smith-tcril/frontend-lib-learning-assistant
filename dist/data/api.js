...
async function fetchChatResponse(courseId, messageList, unitId) {
  ...
  const url = new URL(`${(0, _frontendPlatform.getConfig)().CHAT_RESPONSE_URL}/${courseId}?${queryString}`);
  const {
    data
...
async function fetchLearningAssistantChatSummary(courseId) {
  const url = new URL(`${(0, _frontendPlatform.getConfig)().CHAT_RESPONSE_URL}/${courseId}/chat-summary`);
  const {
    data
